...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
	...
	<form method="post" action="itemInfoUpdatePro.do">
			...
			<tr height="30">
				...
				<td width="300">
					<select name="item_category">
						<c:if test="${ bean.getItem_category() eq '채소' }">selected</c:if>
						<option value="채소" <c:if test="${ bean.getItem_category() eq '채소' }">selected</c:if>>채소</option>
						<option value="해산물" <c:if test="${ bean.getItem_category() eq '해산물' }">selected</c:if>>해산물</option>
						<option value="육류" <c:if test="${ bean.getItem_category() eq '육류' }">selected</c:if>>육류</option>
						<option value="전자제품" <c:if test="${ bean.getItem_category() eq '전자제품' }">selected</c:if>>전자제품</option>
					</select>
				</td>
			</tr>
